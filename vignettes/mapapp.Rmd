...
Heat maps work by mapping a statistic of interest to a color at a set of (x,y) domain locations; call the number of locations $n$. The mapapp dataset `FinalList` uses $n =$ `r nrow(FinalList[[1]])` locations arranged in a evenly spaced grid. `shinyHMCI()` needs data for the point estimate map, and for every CI level---1\% through 99\%. Accordingly, `shinyHMCI()` requires a list with 100 elements, each a data frame with $n$ rows. 
...
As an example I've included the data frame `InitialDF` in `mapapp`:  

```{r, eval = FALSE}
head(InitialDF)
```

In `InitialDF` the columns `px` and `pz` are equispaced locations on a spatial grid.  `phat` represents a point estimate of the parameter of interest (the success probability of a swing at this location).  This particular set of point estimates was constructed with a call to `predict()` from a model fit with `glm()`.
...
```{r, eval = FALSE}
point_estimates <- dplyr::select(InitialDF, 
  x = px, y = pz, stat = phat)
```
...
As an example of the `confidence_intervals` object, take a look at the `?` object provided with mapapp:
```{r, results = "hide"}
str(InitialList[-1], max.level = 1)
```
